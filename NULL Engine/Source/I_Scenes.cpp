...
const aiNode* Importer::Scenes::Utilities::ImportTransform(const aiNode* ai_node, ModelNode& model_node)
{
	...
	// All those transformations will be collapsed onto the first non-dummy node.
	...
	while (NodeIsDummyNode(*ai_node))																		// All dummy nodes will contain the "_$AssimpFbx$_" string and only 1 child node.
	{
		...
		ma_t.position	+= dummy.position;																				// --- Adding the dummy's Transform to the current one.
		ma_t.rotation	= ma_t.rotation * dummy.rotation;																// 
		ma_t.scale		= { ma_t.scale.x * ai_t.scale.x, ma_t.scale.y * ai_t.scale.y , ma_t.scale.z * ai_t.scale.z };	// ----------------------------------------------------
	}
