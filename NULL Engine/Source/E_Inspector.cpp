...
void E_Inspector::DrawComponents(GameObject* selected_game_object)
{
	...
	for (uint i = 0; i < selected_game_object->components.size(); ++i)
	{
		...
		case COMPONENT_TYPE::SCRIPT:	{ DrawScriptComponent((C_Script*)component); }			break;
		}
...
void E_Inspector::DrawScriptComponent(C_Script* c_script)
{
	static bool show = true;
	if (ImGui::CollapsingHeader("Script", &show, ImGuiTreeNodeFlags_DefaultOpen))
	{
		if (c_script != nullptr)
		{
			bool is_active = c_script->IsActive();
			if (ImGui::Checkbox("Is Active", &is_active)) { c_script->SetIsActive(is_active); }

			if (ImGui::BeginCombo("Scripts", ((c_script->GetScript() != nullptr) ? c_script->GetScriptFile() : "Choose a script"), ImGuiComboFlags_None))
			{
				std::vector<Resource*> scriptResources;
				App->resource_manager->GetResourcesOfType(RESOURCE_TYPE::SCRIPT, scriptResources);
				for (Resource* script : scriptResources)
				{
					bool selected = (c_script != nullptr) ? false : (strcmp(c_script->GetScriptFile(), script->GetAssetsFile()) == 0);
					if (ImGui::Selectable(script->GetAssetsFile(), selected, ImGuiSelectableFlags_None))
					{
						c_script->SetScript((R_Script*)script);
					}
				}
			...
			if (ImGui::Button("Recompile"))		{ c_script->OnRecompileScript(); }
			if (ImGui::Button("Recompile All"))	{ c_script->OnRecompileAllScripts(); }
		}

		if (!show)
		{
			component_to_delete			= c_script;
			show_delete_component_popup = true;
		}
	}
}
...
{
	ImGui::Combo("##", &component_type, "Add Component\0Transform\0Mesh\0Material\0Light\0Camera\0Animator\0Animation\0Script");
