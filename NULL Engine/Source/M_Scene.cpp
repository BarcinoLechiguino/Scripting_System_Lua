...
void M_Scene::LoadResourceIntoScene(Resource* resource)
{
	...
	case RESOURCE_TYPE::SCRIPT:		{ success = ApplyScriptToSelectedGameObject(resource->GetUID()); }	break;
	}
}
...
bool M_Scene::ApplyScriptToSelectedGameObject(const uint32& script_UID)
{
	BROFILER_CATEGORY("ApplyNewScriptToSelectedGameObject()", Profiler::Color::Magenta);

	if (selected_game_object == nullptr)
	{
		LOG("[ERROR] Could not add the script to the selected game object! Error: No game object was being selected.");
		return false;
	}
	if (script_UID == 0)
	{
		LOG("[ERROR] Could not add the script to the selected game object! Error: Given Resource UID was 0.");
		return false;
	}

	R_Script* r_script = (R_Script*)App->resource_manager->RequestResource(script_UID);
	if (r_script == nullptr)
	{
		LOG("[ERROR] Could not add the script to the selected game object! Error: R_Script* was nullptr.");
		return false;
	}

	// --- SETTING THE NEW SCRIPT ---
	C_Script* c_script = selected_game_object->GetComponent<C_Script>();									// GetMaterialComponent() == nullptr if GO does not have a C_Material.
	if (c_script == nullptr)
	{
		c_script = (C_Script*)selected_game_object->CreateComponent(COMPONENT_TYPE::SCRIPT);				// Creating a Material Component if none was found in the selected GO.
	}

	c_script->SetScript(r_script);																			// Setting the Material Component's texture with the newly created one.
