...
uint32 M_ResourceManager::ImportFromAssets(const char* assets_path)
{
	...
	if (read > 0)
	{
		...
		case RESOURCE_TYPE::SCRIPT:		{ success = Importer::ImportScript(buffer, (R_Script*)resource); }			break;
		}
...
{	
	...
	case RESOURCE_TYPE::SCRIPT:		{ written = Importer::Scripts::Save((R_Script*)resource, &buffer); }		break;
	}
...
RESOURCE_TYPE M_ResourceManager::GetTypeFromAssetsExtension(const char* assets_path)
{
	...
	else if (extension == "lua" || extension == "LUA")
	{
...
Resource* M_ResourceManager::CreateResource(RESOURCE_TYPE type, const char* assets_path, const uint32& forced_UID)
{
	...
	case RESOURCE_TYPE::SCRIPT:		{ resource = new R_Script(); }		break;
	}
...
Resource* M_ResourceManager::AllocateResource(const uint32& UID, const char* assets_path)
{
	...
	case RESOURCE_TYPE::SCRIPT:		{ success = Importer::Scripts::Load(buffer, (R_Script*)resource); }				break;
	}
...
void M_ResourceManager::GetResourcesOfType(const RESOURCE_TYPE type, std::vector<Resource*>& resourcesOfType) const
{
	for (auto item = resources.cbegin(); item != resources.cend(); ++item)
	{
		if (item->second->GetType() == type)
		{
			resourcesOfType.push_back(item->second);
		}
	}
}